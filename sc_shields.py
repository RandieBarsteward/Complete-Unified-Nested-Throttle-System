...
def onJoystickSwitch_AltModShields(event, vjoy, joy):
    if joy[hotas.JOY_Name].button(hotas.JOYBTN_ShieldFront_Top).is_pressed:
        if event.is_pressed:
            vjoy[1].button(scmap.ShieldFront).is_pressed = False
            vjoy[1].button(scmap.ShieldTop).is_pressed = True
        else:
            vjoy[1].button(scmap.ShieldFront).is_pressed = True
            vjoy[1].button(scmap.ShieldTop).is_pressed = False
    if joy[hotas.JOY_Name].button(hotas.JOYBTN_ShieldBack_Bottom).is_pressed:
        if event.is_pressed:
            vjoy[1].button(scmap.ShieldBack).is_pressed = False
            vjoy[1].button(scmap.ShieldBottom).is_pressed = True
        else:
            vjoy[1].button(scmap.ShieldBack).is_pressed = True
            vjoy[1].button(scmap.ShieldBottom).is_pressed = False
...
def onJoystickBtn_ShieldLeft(event, vjoy):
    vjoy[1].button(scmap.ShieldLeft).is_pressed = event.is_pressed

@joystick.button(hotas.JOYBTN_ShieldRight)
def onJoystickBtn_ShieldRight(event, vjoy):
    vjoy[1].button(scmap.ShieldRight).is_pressed = event.is_pressed
