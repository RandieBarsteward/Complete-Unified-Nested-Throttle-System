...
def setWeapons(vjoy, joy):
    ...
    if joy[hotas.JOY_Name].button(hotas.JOYBTN_FireWep).is_pressed:
        if isAlphaFire == False and joy[hotas.THR_Name].button(hotas.SWITCH_CycleFire).is_pressed:
            if curWepGrp == 1:
                vjoy[15].button(scmap.FireWeaponGroup1).is_pressed = True
                vjoy[15].button(scmap.FireWeaponGroup2).is_pressed = False
            else:
                vjoy[15].button(scmap.FireWeaponGroup1).is_pressed = False
                vjoy[15].button(scmap.FireWeaponGroup2).is_pressed = True
        else:
            vjoy[15].button(scmap.FireWeaponGroup1).is_pressed = True
            vjoy[15].button(scmap.FireWeaponGroup2).is_pressed = True
    else:
...
def onJoystickBtn_CycleWeaponGroup(event, vjoy, joy):
    vjoy[15].button(scmap.CycleWeaponGroup).is_pressed = event.is_pressed
...
def onJoystickHat_Zoom(event, vjoy, joy):
    vjoy[15].hat(1).direction = (event.value)

#@joystick.hat(hotas.JOYHAT_Zoom) # Only UP DOWN on HAT 1
#def onJoystickHat_Zoom(event, vjoy, joy):
#    vjoy[15].hat(1).direction = (0, event.value[1])
